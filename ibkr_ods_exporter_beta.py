...
ASSET_CATS = {
    ...
    "FOP", "WAR", "CMDTY", "BOND" # added bond support to the _beta version
}
...
def process_closing_trades(elements, opens, convert_date=False):
    ...
    while i < len(elements):
        ...
        if asset_category in BONDS:
            close_price = close_price / Decimal("100")
    ...
    for fpath in all_xml_files:
        try:
            ...
            for tx in root.iter('CashTransaction'):
                ...
                interest_transaction_types = ["Broker Interest Received", "Withholding Tax", "Bond Interest Received", "Bond Interest Paid"] # added Bond Interest Received/Paid to the _beta version

                if tx_type not in interest_transaction_types:
                    continue
               
                desc_normalized = tx.get('description', '').upper()  # Normalize case
                ...
                if "SYEP" in desc_normalized and tx_type == "Broker Interest Received":
                    syep_record = {
                        'description': desc,
                        'date': date_formatted,
                        'amount': amount,
                ...
                month_year = extract_month_year(desc_normalized)

                if not month_year:

                    if not transactionID:
                        print(f"WARNING: transactionID is not found for {desc}")
                        month_year = desc
                    else:
                        month_year = transactionID

                elif account_id:
                    month_year = month_year + " " + account_id

                key = (currency, month_year)

                if tx_type == "Broker Interest Received" and "CREDIT INT" in desc_normalized:
                ...
                elif tx_type == "Withholding Tax" and "ON CREDIT INT" in desc_normalized:
                ...
                elif tx_type == "Bond Interest Received":
                    cash_interest_groups[key]['interest'] = {
                        'description': desc,
                        'date': date_formatted,
                        'amount': amount,
                ...
                elif tx_type == "Bond Interest Paid":
                    print(f"WARNING: we ignore Bond Interest Paid (note the difference between Bond Interest Received and Bond Interest Paid): transactionID: {transactionID}, description: {desc}")
                else:
                    print(f"WARNING: This transaction is not processed: transactionID: {transactionID}, description: {desc}")
