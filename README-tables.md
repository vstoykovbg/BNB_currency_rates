...
[!NOTE] Забелязах, че защитата на таблиците (protect sheet) обърква някои потребители. Ако съответният таб не е заключен изначално и изберете Tools -> Protect sheet или десен бутон върху таба -> Protect sheet това ще го заключи вместо отключи и макросът няма да може да го обработи. Ако последната редакция на таблицата съм я оставил незаключена може да я заключите по време на попълването (за да не попълните погрешка полетата, които макросът е предвидено да попълва) и преди да пуснете макроса отключете таблицата. Таблиците с валутните курсове ги оставете заключени ако нямате намерение ръчно да ги коригирате (защо бихте го правили?).
